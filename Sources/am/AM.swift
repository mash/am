...
let AM_VERSION = "0.1.0"
// FOUNDATION_MODEL_SDK_VERSION (the FoundationModels module version present at
// build time) is defined in Version.generated.swift, regenerated by `make` from
// the SDK.

// `am 0.1.0 (FoundationModels framework 1.5.2, macOS 26.5)`.
// FOUNDATION_MODEL_SDK_VERSION is the framework's API version, not the model's;
// macOS version is the only model-version proxy at runtime — the model ships
// with the OS.
func versionString() -> String {
  let v = ProcessInfo.processInfo.operatingSystemVersion
  let os = "\(v.majorVersion).\(v.minorVersion)" + (v.patchVersion > 0 ? ".\(v.patchVersion)" : "")
  return "am \(AM_VERSION) (FoundationModels framework \(FOUNDATION_MODEL_SDK_VERSION), macOS \(os))"
}
